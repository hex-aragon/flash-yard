...
PC 또는 모바일 브라우저에서 `친구랑 방 만들기` → `친구 초대 링크 복사` → 친구에게 링크 공유. 방장을 포함해 최대 8명. 혼자서는 `혼자 몸풀기`로 봇 3명과 연습할 수 있습니다.
...
모바일에서는 왼쪽 조이스틱으로 이동하고 끝까지 밀면 달립니다. 오른쪽 화면을 드래그해 조준하고, 발사 버튼을 누르고 있으면 연사합니다. 점프·재장전·수류탄·섬광탄·메뉴·순위 버튼도 제공합니다. 가로 화면을 권장하며 세로 화면에서도 조작할 수 있습니다. 터치 기기에서는 자동으로 켜지고, 로비의 **화면 조이스틱 · 터치 조작**으로 PC에서도 사용할 수 있습니다. 터치 모드는 마우스 고정 권한이 필요하지 않습니다.
...
- 친구끼리 즐기는 초기 버전입니다. 계정, 영구 전적, 경쟁전용 치트 방지, 전용 서버는 구현하지 않았습니다. 방 코드를 아는 사람은 입장할 수 있습니다.
